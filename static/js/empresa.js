...
let validacion_nit = "no"
let validacion_nombre = "no"
let validacion_email = "no"
let validacion_pais = "no"
let validacion_ciudad = "no"
let validacion_direccion = "no"
let validacion_telefono = "no"
...
nit.addEventListener("input", function(){
    ...
    else if (valor_nit.length != 9){
        error_nit.textContent = "el nit debe tener 9 numeros";
        nit.style.border = "solid 1px red";
        error_nit.style.color = "#f00";
        validacion_nit = "no"
        activarBoton()
...
formulario.addEventListener('submit', async function (event) {
    ...
    const id = document.getElementById("nit").value
    // Petición GET al microservicio de productos
    const response = await fetch("/verificarNitEmpresa/"+id, { method: 'GET' });
    const respuesta = await response.json();
    
    if (respuesta.mensaje == "si"){
        event.target.submit()
    }else{
        const campo_error_nit = document.getElementById("error_nit")

        campo_error_nit.textContent = "la empresa ya existe"
        campo_error_nit.style.color = "red"
        document.getElementById("nit").style.border = "solid 1px red"
    }
